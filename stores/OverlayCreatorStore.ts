...
class OverlayCreatorStore {
  ...
  initializeVideoPlayer(htmlElement: HTMLDivElement, newSource: any) {
    if (this.preview) {
  ...
  async applyTemplate() {
    ...
    deepClone(this.template).map((templateElement) => {
      ...
      templateElement.height = Math.round(scaler * templateElement.height)
      templateElement.width = Math.round(scaler * templateElement.width)
